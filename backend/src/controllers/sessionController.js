...
export async function createSession(req, res) {
    try {
        ...
        if (!problem || !difficulty) {
            return res.status(400).json({ msg: "Problem and difficuty required" });
...
export async function getMyRecentSessions(req, res) {
    try {
        ...
        console.error("Error in getMyRecentSession controller:", error.message);
        res.status(500).json({ msg: "InternalServer Error" });
    }
}
...
export async function joinSession(req, res) {
    try {
        ...
        if (session.status !== "active") {
            return res.status(400).json({ msg: "Cannot join inactive session" });
        }
...
export async function endSession(req, res) {
    try {
        ...
        console.error("Error in the endSession controller:", error.message);
        res.status(500).json({ msg: "Internal Server Error" })
    }
}